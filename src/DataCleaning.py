...
def clean_data(file):
    ...
    # Make the marks of absent students to zero since they were absent for the marks and hence their marks cannot be adjust the any measures of dispersion

    absent_students = df.apply(lambda row: 'AB' in row.values, axis=1)
    df.loc[absent_students] = df.loc[absent_students].replace('AB', 0)

    # The same applies for copy case as absent students
