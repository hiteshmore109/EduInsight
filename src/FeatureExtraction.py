...
def get_sgpa(sem_data):
    ...
    practical_columns = [col for col in sem_data.columns if col.endswith(('Pint', 'Prac'))]
    sem_data[subject_columns] = sem_data[subject_columns].apply(pd.to_numeric, errors='coerce')
    sem_data[practical_columns] = sem_data[practical_columns].apply(pd.to_numeric, errors='coerce')
    credits_df = pd.DataFrame()

    for practical_column in practical_columns:
        practical_name = practical_column.split()[0]
        practical_name += "Prac"
        total_marks = sem_data[practical_column] + sem_data[practical_column.replace('Prac', 'Pint')]
    ...
    for subject_column in subject_columns:
        ...
        total_marks = sem_data[subject_column] + sem_data[subject_column.replace('Ext', 'Int')]
        gpa = calculate_credits(total_marks)
